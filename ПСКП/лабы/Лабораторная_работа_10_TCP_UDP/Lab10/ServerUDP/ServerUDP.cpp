...
int main()
{
	...
	try
	{
		...
		while (true)
		{
			...
			while (true)
			{
				if ((recvfrom(serverSocket, ibuf, sizeof(ibuf), 0, (LPSOCKADDR)&client, &lClient)) == SOCKET_ERROR)
				{
					throw  SetErrorMsgText("recvfrom:", WSAGetLastError());
				}

				Sleep(3000);
				std::cout << std::endl << "Client message: " << ibuf;

				char obuf[50] = "ECHO ";
				strcat_s(obuf, ibuf);
				if ((sendto(serverSocket, obuf, strlen(obuf), 0, (LPSOCKADDR)&client, sizeof(client))) == SOCKET_ERROR)
